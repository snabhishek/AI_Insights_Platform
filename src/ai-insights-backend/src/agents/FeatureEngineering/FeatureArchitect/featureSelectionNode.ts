...
export async function featureSelectionNode(
  ...
  const userMessage = [
    ...
    `Prediction Target Column: ${state.prediction_target_column || state.orchestrationDecision?.targetColumn || "None"} (CRITICAL: Preserved target column, exclude from removal/filtering)`,
