...
3. Check `artifacts/models/preprocessor.joblib` in the training directory. If it exists, check its `feature_names_in_`.
4. When feature count mismatch occurs (e.g., model expects 49 features, but input data has 46 features):
   - **DO NOT** create a new, ad-hoc preprocessor or dummy scaler in `validation_runner.py` that generates a different number of features.
   - **DO** load `artifacts/models/preprocessor.joblib` and use `preprocessor.transform(df)`.
   - Ensure the raw input DataFrame passed to `preprocessor.transform()` contains all the raw feature columns present during training (`preprocessor.feature_names_in_`).
   - If engineered features (e.g. temporal, lag, rolling) are expected, ensure they are generated with exact matching column names before passing to the preprocessor or model.
5. Identify missing, unexpected, duplicated, or incorrectly ordered columns.
6. Recommend a safe feature-alignment strategy:
   - If the estimator has `feature_names_in_`, reorder and align DataFrame columns: `X = X[estimator.feature_names_in_]`.
   - If columns are missing, check if they are categorical one-hot encoded or cyclical features generated during training and construct them accordingly.
...
1. Inspect how `preprocessor.joblib` is loaded and applied. Verify that `artifacts/models/preprocessor.joblib` is loaded from the active training directory (`--models-dir`).
2. Verify the expected input schema. Check `preprocessor.feature_names_in_`.
3. Check categorical, numerical, temporal, and engineered features.
4. If `preprocessor.joblib` was generated by scikit-learn (`ColumnTransformer` or `Pipeline`), call `preprocessor.transform(X)` rather than `fit_transform(X)` to preserve the exact learned encoders, scalers, and column dimensions.
5. If `preprocessor.joblib` is missing from the training artifacts, inspect `model_training/data/data_loader.py` or `model_training/models/pipeline.py` to replicate the exact training feature engineering and transformation pipeline.
...
### 6.7 Missing Packages and Environment Errors

If a missing package is confirmed by the traceback or source inspection (e.g. `ModuleNotFoundError`, `ImportError`, or container execution failure):

- Identify the exact package name (e.g. `lightgbm`, `xgboost`, `catboost`, `scikit-learn`, `torch`, `statsmodels`, `neuralforecast`).
- Add it to `requiredPackages` in your JSON output.
- Check `requirements.txt` in the training directory (`<runTimestamp>/<projectName>_model_training/requirements.txt`) and include any packages used during training that validation also requires.
- Ensure the package names are standard PyPI distribution names (e.g. `scikit-learn` instead of `sklearn`).
...
9. **CRITICAL**: In `validation_runner.py`, do NOT catch exceptions per model and silently exit with exit code 0 if ALL candidate models failed! If zero models succeed, `validation_runner.py` MUST exit with a non-zero code (e.g. `sys.exit(1)`) so the self-healing retry loop detects the failure immediately.
10. Avoid producing a false successful validation result where `metrics: {}` or all metrics are null/N/A.
