...
export default function TrainingConfigurationStepOutput({
  ...
  const fetchContract = useCallback(async () => {
    ...
    setErrorMsg(null);
    try {
      const url = `${BACKEND_URL}/training-config/${projectId}${activeRunTimestamp ? `?timestamp=${encodeURIComponent(activeRunTimestamp)}` : ""
        }`;
  ...
  const fetchDateRange = useCallback(async () => {
    ...
    try {
      ...
      console.warn("[TrainingConfig] Failed to fetch date range:", err);
    } finally {
  ...
  const handleApproveWithSplitDate = () => {
    if (!onApprove) return;
    const formattedSplit = `${selectedYear}-${String(selectedMonth).padStart(2, "0")}`;
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-2xl border border-border bg-gradient-to-r from-surface to-surface-muted shadow-xs">
        ...
        <div className="flex items-center bg-surface-muted border border-border p-1 rounded-xl shadow-2xs self-stretch sm:self-auto justify-center">
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${activeTab === "view"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${activeTab === "edit"
                ? "bg-surface text-foreground shadow-xs font-bold"
                : "text-muted-foreground hover:text-foreground"
              }`}
          >
        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-border/70">
              ...
              <div className="flex items-center gap-2 self-start md:self-auto shrink-0 flex-wrap">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer shadow-xs ${hasSelectionChanged
                ...
                {candidatePool.map((model: any, index: number) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative p-4 rounded-xl border transition-all cursor-pointer flex flex-col justify-between select-none ${isSelected
                      ...
                      <div>
                        <div className="flex items-start justify-between gap-2 mb-2.5">
                          <div className="flex items-center gap-2.5">
                            <div
                              className={`w-4 h-4 rounded-md border flex items-center justify-center transition-all ${isSelected
          ...
          <div className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/5 via-surface to-surface p-5 shadow-xs space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground flex items-center gap-2">
                    Model Training Temporal Split Cutoff
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Records up to the selected Split End Date will be used for training candidate models. Remaining data will be evaluated.
                  </p>
                </div>
              </div>

              {dateRangeInfo?.timeColumn && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface border border-border shadow-2xs">
                  <span className="text-[10px] uppercase font-bold text-muted-foreground">Time Column:</span>
                  <span className="text-xs font-mono font-bold text-primary">{dateRangeInfo.timeColumn}</span>
              ...
              <div>
                ...
                >
                  {MONTH_NAMES.map((name, idx) => {
                    ...
                    const isBeforeMin = isMinYear && dateRangeInfo?.minMonth != null && monthNum < dateRangeInfo.minMonth;
            ...
            <div className="p-3 rounded-xl bg-surface/80 border border-border/80 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-foreground font-mono bg-surface-muted px-2 py-0.5 rounded border border-border">
                  {selectedYear}-{String(selectedMonth).padStart(2, "0")} ({MONTH_NAMES[selectedMonth - 1]} {selectedYear})
                </span>
              </div>
              <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold">
                ✓ Dynamic range verified from dataset
              </span>
            </div>
          </div>
          ...
          {onApprove && (
            <div className="rounded-xl border border-border bg-gradient-to-r from-surface to-surface-muted p-5 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-foreground">
                    Training Contract Verified & Configured
                  </p>
                  <p className="text-[11px] text-muted-foreground">
                    Approve to run automatic pre-flight verification and advance to model training.
                  </p>
                </div>
              </div>
              ...
              <button
                type="button"
                disabled={isApproving || isSaving || isApplyingModels}
                onClick={handleApproveWithSplitDate}
              ...
              >
                {isApproving ? (
                  ...
                  <>
                    ...
                    <span>Approve & Start Preflight</span>
                  </>
                )}
              </button>
            </div>
